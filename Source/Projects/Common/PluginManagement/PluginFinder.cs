...
namespace SoftwarePassion.Common.Core.PluginManagement
{
    ...
    public static class PluginFinder
    {
            ...
            where TImplementee : class
        {
            ...
            if (composerTypes.Count() > 1)
            {
                ...
                if (composerTypes.Count() > 1)
                {
                    ...
                    throw new InvalidOperationException(
                        $"Too many ({composerTypes.Count()}) implementations of '{typeof(TImplementee).FullName}' found: '{foundTypes}'. ");
                }
            }
            ...
            {
                throw new InvalidOperationException($"No implementation for '{typeof(TImplementee).FullName}' found.");
            }
