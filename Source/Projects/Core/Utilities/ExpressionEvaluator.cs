...
namespace SoftwarePassion.Common.Core.Utilities
{
    ...
    public class ExpressionEvaluator
    {
        ...
        /// <param name="valuesValue">The values value, first index is A, second is B and so on.</param>
        /// <returns>A boolean value representing the result</returns>
        public bool Evaluate(string expressionValue, bool[] valuesValue)
        {
            expression = expressionValue;
            criteriaValues = valuesValue;
            return Eval();
        }
        ...
        private bool Next()
        {
            ...
            else if (token[0] >= 'A' && token[0] <= 'Z')
            {
                ...
                if (valueIndex < criteriaValues.Length && valueIndex >= 0)
        ...
        bool Eval()
        {
            ...
            {
                int index;
                for (index = 0; index < criteriaValues.Length; index++)
                {
                    if (criteriaValues[index] == false)
                        return false;
                }
                return true;
            }
